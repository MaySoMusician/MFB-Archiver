...
exports.run = (MFBAcvr, message, args) => {
  // If no specific command is called, show all filtered commands.
  if(!args[0]) {
    // Filter all commands by which are available for the user's level, using the <Collection>.filter() method.
    const myCommands = message.guild
    ? MFBAcvr.commands.filter(cmd => MFBAcvr.levelCache[cmd.conf.permLevel] <= message.author.permLevel)
    : MFBAcvr.commands.filter(cmd => MFBAcvr.levelCache[cmd.conf.permLevel] <= message.author.permLevel && cmd.conf.guildOnly !== true);
    ...
    let currentCategory = "";
    let output = `= コマンド一覧 =\n\n[${MFBAcvr.config.prefix}help <コマンド名> で詳細表示]\n`;
    ...
    sorted.forEach( c => {
      ...
      output += `${MFBAcvr.config.prefix}${c.help.name}${" ".repeat(longest - c.help.name.length)} :: ${c.help.description}\n`;
    });
    message.channel.send(output, {code: "asciidoc", split: { char: "\u200b" }});
  } else {
    // Show individual command's help.
    let command = args[0];
    if(MFBAcvr.commands.has(command)) {
      command = MFBAcvr.commands.get(command);
      if (message.author.permLevel < MFBAcvr.levelCache[command.conf.permLevel]) return; // if you don't have enough permission, you'll be ignored
